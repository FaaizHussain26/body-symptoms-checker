...
export interface Treatment {
  id: string
  name: string
  type: "medication" | "lifestyle" | "therapy" | "home_remedy" | "medical_procedure"
  description: string
  severity: "mild" | "moderate" | "severe"
  instructions: string
  precautions?: string
...
interface SymptomsState {
  currentStep: "demographics" | "body_selection" | "results"
...
const symptomsReducer = (state: SymptomsState, action: SymptomsAction): SymptomsState => {
  switch (action.type) {
    case "SET_DEMOGRAPHICS":
      return {
        ...state,
        userDemographics: action.payload,
        currentStep: "body_selection",
      }
    case "GO_TO_RESULTS":
      return {
        ...state,
    ...
    case "TOGGLE_SYMPTOM":
      const existingIndex = state.selectedSymptoms.findIndex(
        (s) => s.bodyPart === action.payload.bodyPart && s.symptomId === action.payload.symptom.id,
      )

      if (existingIndex >= 0) {
        return {
          ...state,
          selectedSymptoms: state.selectedSymptoms.filter((_, index) => index !== existingIndex),
        }
      } else {
        return {
          ...state,
...
export const SymptomsProvider = ({ children }: { children: ReactNode }) => {
  ...
  return <SymptomsContext.Provider value={{ state, dispatch }}>{children}</SymptomsContext.Provider>
}
