...
export const SelectedSymptomsPanel: React.FC = () => {
      ...
      <Card className="mt-6">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-gray-600">
            Click on body parts to select symptoms
          </p>
        </CardContent>
      </Card>
    );
  ...
  const groupedSymptoms = state.selectedSymptoms.reduce((acc, selected) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-xl">
              Selected Symptoms ({state.selectedSymptoms.length})
            </CardTitle>
            <Button
              variant="outline"
              size="sm"
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-600">
                          {selected.symptom.description}
                        </p>
                      </div>
                      <div className="ml-4">
                        <Badge
                          className={getPercentageColor(
                            selected.symptom.percentage
                          )}
                          variant="outline"
                        >
      ...
      <Card
        className={`border-2 ${getRiskLevelColor(riskCalculation.riskLevel)}`}
      >
        <CardHeader>
          <div className="flex items-center justify-between">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-center p-4 bg-white rounded-lg border">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Based on {riskCalculation.totalSymptoms} symptoms (Avg:{" "}
              {riskCalculation.averageRisk}%)
            </p>
          </div>
              ...
              <div className="space-y-1">
                {riskCalculation.highestRiskSymptoms.map((symptom, index) => (
                  ...
                  >
                    ...
                    <Badge
                      className={getPercentageColor(symptom.percentage)}
                      variant="outline"
                      // size="sm"
                    >
      ...
      {state.selectedSymptoms.length >= 2 && (
        <div className="p-4 bg-blue-50 rounded-lg">
          ...
          <p className="text-xs text-blue-600 mt-1">
            You have selected {state.selectedSymptoms.length} symptoms with{" "}
            {riskCalculation.overallRiskPercentage}% overall risk score.
          </p>
          <Button
