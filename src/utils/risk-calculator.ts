...
export function calculateOverallRisk(selectedSymptoms: SelectedSymptom[]): RiskCalculation {
  ...
  const averageRisk =
    totalSymptoms > 0 ? selectedSymptoms.reduce((sum, s) => sum + s.symptom.percentage, 0) / totalSymptoms : 0
  const overallRiskPercentage = Math.min(100, totalWeightedRisk / totalWeight)
